...
export function CO2Calculator() {
  ...
  // Compute everything when calculated
  const result = useMemo(() => {
    ...
    // Build all transport option display data
    ...
    <section id="calculator" className="py-24 px-6 bg-background">
      <div className="mx-auto max-w-7xl">
        <div className="text-center mb-16">
          <Badge variant="secondary" className="mb-4">Carbon Impact Calculator</Badge>
        ...
        <Card className="border-border bg-card mb-10">
          <CardContent className="p-6 md:p-8">
            ...
            <div className="grid gap-6 md:grid-cols-2 mb-6">
              <div>
                <Label className="text-card-foreground flex items-center gap-2 mb-2">
            ...
            <div className="mb-6">
              <Label className="text-card-foreground mb-3 block">Transport Mode</Label>
            ...
            <div className="grid gap-6 md:grid-cols-2 mb-8">
              <div>
                <Label className="text-card-foreground flex items-center justify-between mb-3">
                  <span className="flex items-center gap-2"><Users className="h-4 w-4 text-primary" /> Travelers</span>
              ...
              <div>
                <Label className="text-card-foreground flex items-center justify-between mb-3">
                  <span className="flex items-center gap-2"><Calendar className="h-4 w-4 text-primary" /> Duration (days)</span>
        ...
        {result && (
          <div className="flex flex-col gap-8">
            ...
            <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
              {[
                { label: "Distance", value: `${result.distance} km`, color: "text-sky-500" },
                { label: "Travelers", value: travelers.toString(), color: "text-primary" },
                { label: "Duration", value: `${days} days`, color: "text-accent-foreground" },
                ...
                <Card key={m.label} className="border-border bg-card">
                  <CardContent className="p-5 text-center">
                    ...
                    <p className={`font-serif text-2xl font-bold ${m.color}`}>{m.value}</p>
                  </CardContent>
                </Card>
              ))}
            </div>
            ...
            <Card className={`border-2 ${result.isAlreadyEco ? "border-primary bg-primary/5" : "border-destructive/30 bg-destructive/5"}`}>
              <CardContent className="p-6 flex items-center gap-4">
                <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full ${result.isAlreadyEco ? "bg-primary/10" : "bg-destructive/10"}`}>
                  <Leaf className={`h-6 w-6 ${result.isAlreadyEco ? "text-primary" : "text-destructive"}`} />
                </div>
                <div>
                  <p className="font-semibold text-card-foreground">Eco Assistant</p>
                  {result.isAlreadyEco ? (
              ...
              <Card className="border-border bg-card">
                <CardContent className="p-6">
                  <div className="flex items-center gap-2 mb-6">
                    ...
                    <h3 className="font-semibold text-card-foreground">Your Current Plan</h3>
                  </div>
                  ...
                  <div className="rounded-lg bg-destructive/10 p-4">
                    <p className="text-lg font-bold text-destructive">Total: INR {formatCurrency(result.userTotalCost)}</p>
                    <p className="text-sm text-destructive/80">Carbon: {Math.round(result.userTotalCo2)} kg CO2</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-primary/30 bg-primary/5">
                <CardContent className="p-6">
                  <div className="flex items-center gap-2 mb-6">
                    ...
                    <h3 className="font-semibold text-card-foreground">Recommended Eco Plan</h3>
                    <Badge className="ml-auto bg-primary text-primary-foreground border-0">Best</Badge>
                  </div>
                  <div className="flex flex-col gap-3 mb-6">
                  ...
                  <div className="rounded-lg bg-primary/10 p-4">
                    <p className="text-lg font-bold text-primary">Total: INR {formatCurrency(result.ecoBest.totalCost)}</p>
                    <p className="text-sm text-primary/80">Carbon: {Math.round(result.ecoBest.totalCo2)} kg CO2</p>
                  </div>
                    ...
                    >
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          {isRecommended && <Badge className="bg-primary text-primary-foreground border-0 text-[10px] px-1.5 py-0.5">Eco</Badge>}
                          {isCurrent && !isRecommended && <Badge variant="secondary" className="text-[10px] px-1.5 py-0.5">You</Badge>}
                        </div>
                        <p className="text-xs text-muted-foreground mb-3 truncate">{opt.name}</p>
                        <div className="flex flex-col gap-1 text-sm">
                          <span className="text-foreground">INR {formatCurrency(opt.cost)}</span>
                  ...
                  <Card key={i} className={`border-2 ${i === 0 ? "border-primary bg-primary/5" : "border-border bg-card"}`}>
                    <CardContent className="p-5">
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-border text-sm">
                        <span className="text-foreground font-semibold">INR {formatCurrency(combo.totalCost)}</span>
